...
transVar_declaration x = case x of
  Var_declaration1 varspecs type_ -> do
    let (str, exs) = transVar_specs varspecs
    M_var (str, exs, transType type_)
  
transVar_specs :: Var_specs -> (String,[M_expr])
transVar_specs x = case x of
  Var_specs1 varspec morevarspecs -> (transVar_spec varspec) : (transMore_var_specs morevarspecs)
  
transMore_var_specs :: More_var_specs -> [(String,[M_expr])]
...
transVar_spec :: Var_spec -> (String,[M_expr])
transVar_spec x = case x of
  Var_spec1 ident arraydimensions -> (transIdent ident, transArray_dimensions arraydimensions)
...
transMore_cons_decl x = case x of
  More_cons_decl1 consdecl moreconsdecl -> (transCons_decl consdecl) : (transCons_declarations moreconsdecl)
  More_cons_decl2 -> []
...
transInt_term x = case x of
  ...
  Int_termInt_factor intfactor -> intfactor
...
transInt_factor x = case x of
  ...
  Int_factor2 ident basicarraydimensions -> M_size (transBasic_array_dimensions basicarraydimensions)
